...
type StaffEditScreenRouteProp = RouteProp<HomeStackParamList, 'EditStaff'>;

const StaffEditScreen: React.FC = () => {
    const navigation = useNavigation();
    const route = useRoute<StaffEditScreenRouteProp>();
    ...
    const loadStaffDetails = async () => {
        try {
            ...
            Alert.alert('Error', error.message || 'Failed to load staff details');
            navigation.goBack();
        } finally {
    ...
    const populateForm = (staff: StaffMember) => {
        setFormData({
            ...
            designation: staff.designation || Designation.TEACHER,
            department: staff.department || '',
            employment_type: staff.employment_type || EmploymentType.PERMANENT,
            employment_status: staff.employment_status || EmploymentStatus.ACTIVE,
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        try {
            // Reconstruct payload. Note: first_name/last_name might need to be in a nested 'user' object 
            // depending on how staffService.updateStaffMember handles it.
            // Based on models.ts, StaffMember has a 'user: User' field.

            const payload: any = {
                ...formData,
                // The backend likely expects user updates here or handles it via staff update
                user: {
    ...
    if (initialLoading) {
        return (
            <ScreenWrapper>
                <Header title="Edit Staff" showBackButton />
